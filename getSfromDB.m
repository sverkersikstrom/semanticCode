...
persistent addNewFunction 
persistent d;
%w='i';sTmp=getSfromDB(initSpace,'sv','test99',{'_ref1'},{w},'update');i=word2index(sTmp,w);sTmp.fwords{i};sTmp.x(i,1:10)
%t{8}='accomplish';fetch(getDb,['select `id`,`xdata`,`info`,`f`,`wordclass` from  `space2`.`spaceEnglish` where   `id` = ''' t{8} ''''  ])
...
if strcmpi(addNewFunction,lang)==0
    try
        newfunction='_liwcseeing';
        query=['SELECT `id` FROM `' lang '` WHERE `id` LIKE ''' newfunction ''''];
        r=fetch(getDb,query);
        if isempty(r)
            s2=initSpace;
            s2.languagefile=lang;
            s2.par.db2space;
            s2.data=0;
            s2=spaceAddModels(s2,1);
            dbSpace(s2.languagefile,'','save',s2.fwords,s2.x,s2.info,'w',s2.f,s2.wordclass);
            delete(file)
        end
        %addNewFunctions2dB(lang,file);
        addNewFunction=[addNewFunction {lang}];
    catch
        fprintf('Failed in adding functions to %s\n',file)
    end
end
...
elseif useCach & exist(file)
    ...
    if exist(initfile)
        ...
        if not(isfield(s,'xmean2'))
            [s2.x,s2.info,s2.f,s2.fwords,s2.wordclass] =dbSpace(lang,'','get',{'_xmean2','_nwords','_nwordsfound','_frequency','_predvalence','_predvalencestenberg','_associates','_furthestassociates'});
            s=mergeSpace(s,s2);
...
%         
%         dbSpace(s.languagefile,'','save',s.fwords,s.x,s.info,'w',s.f,s.wordclass);
