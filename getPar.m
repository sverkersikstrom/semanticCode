...
par.trainType='oneByOne';%options oneByOne multipleInput%Type of predictions
...
par.preprocessWithPCA=0;%options 0 1%Preprocesses training data with PCA
...
par.contextPrintLabels=0;%options 0 1%Print out labels on the semantic dimensions
par.numericalData=[];
par.contextFindstr='';%options words sentence%
par.contextFindstr=0;%Extract context for parts of words 
par.contextType='word';%options word sentence%Units of extraction of context size
...
par.sortwordcrit=2;
par.getPropertyMultipleInput=0;
par.getPropertyImped=0;
...
par.plotNominal='non-nominal';%options nominal non-nominal%Treat data as nominal (categories) or non-nominal (continous)
...
par.plotRemoveOutliers=0;%Remove outliers (+/-z=5)
...
par.selectBestDimensions='noSelection';%options noSelection stepwise simple after%Select dimensions orded by how well they correlate training with outcome variables
...
par.trainBinaryOptimizeMethod='recall&precision';%options recall&precision recall FA Hits%Optimize threshold for binary outputs
...
par.testGroupCondition='';%Selection test conditions during leave-out,i.e. _subject<=420
par.trainSaveData=0;
par.trainSaveModelinLanguageFile=0;%options 0 1%Save the model the language file
par.trainAddStepwise2results=1;
...
par.LSAaddColumnNames=0;%options 0 1%Add column names to LSA inputs
par.LSAaddRowNames=0;%options 0 1%Add row name to cells to LSA inputs
par.LSAaddNumeric=0;%options 0 1%Add row numeric data to LSA inputs
...
global rootPath;
par.rootPath=rootPath;

%category BERT
par.BERT='LSA';%options LSA bert_base_multilingual_cased bert_base_uncased%Use BERT model instead of LSA
par.DLATK=0;%options 0 1%Use DLATK (not used)
