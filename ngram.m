...
elseif 1 | strcmpi(d.func,'createSpace')
    ...
    if d.ngramfile
        ...
        [c,s]=file2coOrruence(d,d.file,s);
        %Create space
...
if f==-1
    ...
    fileExcel=regexprep(file,'.txt','.xlsx');
    if exist([d.path fileExcel])
        fprintf('Creating file: %s\n',[d.path fileExcel]);
        [text, ~,~, labels]=textread2([d.path fileExcel]);
        cell2file(text,file,labels);
        f=fopen([d.path file],'r','n', 'UTF-8');
    else
        s=[];
        return
...
s.nchar=0;
s.nWords=0;
s.filename=['lang ' regexprep(file,'.txt','')];%d.path 
...
function s=word2dictionary(s,data);
Nlarge=480000;
...
function [c,s]=file2coOrruence(d,file,s);
...
if c.multi==0
    ...
    if strcmpi(d.inputType,'likes')
        ...
        while not(feof(f)) & (d.debug==0 | c.i<d.debugN)
            ...
            row=row+1;
            c.feof=feof(f);
            if s.par.LSAtabSeperator | s.par.LSAaddColumnNames | s.par.LSAaddRowNames | s.par.LSAaddNumeric
                if s.par.LSAaddRowNames | s.par.LSAaddColumnNames | s.par.LSAaddNumeric
                    text=[regexprep(text,'_','') ' '];
                end
                text=textscan(text,'%s','delimiter',char(9));text=text{1};
                if s.par.LSAaddRowNames & length(text)>0
                    rowWord=['row' num2str(row)];
                    s=word2dictionary(s,rowWord);
                    %row=[text{1} ' '];
                else
                    rowWord='';
                end
                if s.par.LSAaddNumeric
                    numeric=str2double(text');
                else
                    numeric=NaN(1,length(text));
                end
                for i=1:length(text)
                    if s.par.LSAaddColumnNames | s.par.LSAaddRowNames | s.par.LSAaddNumeric
                        if row==1
                            text=regexprep(text,'_',' ');
                            columns=text;
                            s=word2dictionary(s,cell2string(columns));
                        elseif isnan(numeric(i)) & (s.par.LSAaddColumnNames | s.par.LSAaddNumeric)
                            s=word2dictionary(s,rowWord);
                            ...
                            c=text2coOcurrence(c,s,[rowWord ' ' col ' ' text{i}],d,numeric,columns);
                        elseif s.par.LSAaddNumeric 
                            1;%Ignore numeric data
                        end
                    else
                        c=text2coOcurrence(c,s, [rowWord text{i}],d);
                    end
                end
            else
...
elseif not(isempty(strfind(d.file,'/')))
    i=strfind(d.file,'/');
    d.path=d.file(1:i(end));
    d.file=d.file(i(end)+1:end);
    d2.file=d.file;
else
...
function c=text2coOcurrence(c,s,text,d,numeric,columns)
if nargin<5
    numeric=NaN;
    columns=[];
end
...
try
    ...
    if quick & not(isfield(c,'tmpCount'))
        ...
        c.Numeric=spalloc(size(c.count,1),size(c.count,2),d.memorySize);
        c.NumericN=spalloc(size(c.count,1),size(c.count,2),d.memorySize);
    end
    ...
    nr=1:length(index);
    if max(index)>size(c.tmpCount,1)
        c.tmpCount(max(index)+100,size(c.tmpCount,2))=0;
        c.count(max(index)+100,size(c.count,2))=0;
        c.Numeric(max(index)+100,size(c.count,2))=0;
        c.NumericN(max(index)+100,size(c.count,2))=0;
    end
    if max(index)>size(c.tmpCount,2)
        c.tmpCount(size(c.tmpCount,1),max(index)+100)=0;
        c.count(size(c.count,1),max(index)+100)=0;
        c.Numeric(size(c.count,1),max(index)+100)=0;   
        c.NumericN(size(c.count,1),max(index)+100)=0;   
    end
    if s.par.LSAaddNumeric
        indexNumeric=find(not(isnan(numeric)));
        [indexNumericC wordsNumeric]=text2index(s,cell2string(columns(indexNumeric)));
        indexNotNan=index(find(not(isnan(index))));
        if not(isnan(indexNumericC))
            c.Numeric(indexNotNan,indexNumericC)=c.Numeric(indexNotNan,indexNumericC)+numeric(indexNumeric);
            c.NumericN(indexNotNan,indexNumericC)=c.NumericN(indexNotNan,indexNumericC)+add;
        end
    end
    ...
    if even(c.i-1,10000) | c.feof | abs(lastNow-now)*24*60>1
        ...
        c.Nsave=c.Nsave+1;
        if quick
            if size(c.tmpCount,1)<size(c.count,1); c.tmpCount(size(c.count,1),size(c.count,2))=0;end
            c.count=c.count+c.tmpCount;
            c.tmpCount=spalloc(size(c.count,1),size(c.count,2),d.memorySize);
        end
        if s.par.LSAaddNumeric
            indexNumeric=find(sum(c.NumericN)>0);
            indexNumeric2=sum(c.NumericN')>0;
            c.count(indexNumeric2,indexNumeric)=c.Numeric(indexNumeric2,indexNumeric)./max(1,c.NumericN(indexNumeric2,indexNumeric));
            for i=1:length(indexNumeric)
                c.count(indexNumeric2,not(indexNumeric(i)))=nanmean(nanmean(c.count(indexNumeric2(i),indexNumeric)));
            end
             c.count(indexNumeric,indexNumeric2)= c.count(indexNumeric2,indexNumeric)';
        end
