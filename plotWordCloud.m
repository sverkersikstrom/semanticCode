...
if 0
    ...
    par.plotNominal='';%options non-nominal nominal%Type of word cloud plot
...
if nargin<1 %Get user inputs
    ...
    if isfield(s.par,'plotNumber') & s.par.plotNumber>0
        ...
        %h=5;
        try;
            %fprintf('Trying to closing figure\n')
            %h=figure(5);
            %fprintf('Trying to closing figure\n')
            close(5);
        catch
...
if strcmpi(s.par.plotNominal,'nominal')
    data{1}.f=f1;
    data{1}.word=word1;
    ...
    y=numbers{1};
    if iscell(y)
        fprintf('Converting nomincal text data to integers\n')
        s.par.plotNominalLabels=unique(y);
        for i=1:length(s.par.plotNominalLabels)
            yCon(find(strcmp(s.par.plotNominalLabels{i},y)))=i;
        end
        y=yCon;
        numbers{1}=y;
    end
    ...
    d=wordCloud(s,d,data{1}.word,nanmin([data{1}.p; data{2}.p; data{3}.p]),fontsize,show,plotAxis(data{1},par{1}),plotAxis(data{2},par{2}),plotAxis(data{3},par{3}),labels,par,nanmax([data{1}.zOrg; data{2}.zOrg; data{3}.zOrg]),y);
...
try
    ...
    out.dataTabel=sprintf('word\tN\tclass\t');
    for k=1:3
        if max(1,Ndim)>=k
            out.dataTabel=[out.dataTabel sprintf('x[%d]\tp[%d]\t%s[%d]\t',k,k,data{1}.scale,k)];
        end
    end
    %if Ndim>=2
    ...
    %[~,indexSort]=sort(data{1}.f,'descend');
     if length(s.par.plotWordcountMaxNumber)>=3
         if 1 %Select by frequency on third paramter
             data{1}.zOrg(data{1}.f<s.par.plotWordcountMaxNumber(3))=median(data{1}.zOrg);
         else %Select by rank order on third paramter
             [tmp indexF]=sort(fontsize,'descend');
             data{1}.zOrg(indexF(s.par.plotWordcountMaxNumber(3)+1:end))=median(data{1}.zOrg);
         end
     end
    [~,indexSort]=sort(data{1}.zOrg,'descend');
    if length(s.par.plotWordcountMaxNumber)>=2
        indexSort=[indexSort(1:s.par.plotWordcountMaxNumber)  indexSort(end-s.par.plotWordcountMaxNumber+1:end)];
    end

    if length(y)<length(data{1}.word);y=NaN(1,length(data{1}.word));end
    for j=1:length(indexSort) %data{1}.f
        i=indexSort(j);
        out.dataTabel=[out.dataTabel sprintf('%s\t%.4f\t%d\t',data{1}.word{i},full(data{1}.f(i)),y(i))];
        for k=1:3
            if max(1,Ndim)>=k
                out.dataTabel=[out.dataTabel sprintf('%.4f\t%.4f\t%.4f\t',full(data{k}.z(i)),full(data{k}.p(i)),full(data{k}.zOrg(i)))];
            end
        end
        %if Ndim>=2
...
if s.par.plotAutoSaveFigure
    ...
    figFile=['figure/Figure ' regexprep(s.languagefile,'.mat','') '-' s.par.variableToCreateSemanticRepresentationFrom '-' s.par.plotTestType '-' labels2 '-' s.par.plotCloudType plotCategory s.par.condition_string];
...
if (length(data.z)==0 | nanstd(data.p)==0 | isnan(nanmean(data.z))) & nargin<4
    ...
    if  strcmpi(data.par.plotNominal,'nominal')
         text=sprintf('The figure show nominal data'); 
    else
...
if s.par.plotWordcloud
    if isempty(numbers) & not(strcmpi(par.plotTestType,'property'))
        ...
        data.z=0*data.z;
        data.medianSplit=0
    elseif strcmpi(par.plotTestType,'semanticTest') 
        data.z(data.z< 0)=-1;
        data.z(data.z>=0)=+1;
    else
