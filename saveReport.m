function reportFilename=saveReport(s,datafile)
if nargin<1
    s=getSpace;
end
if nargin==2
...
if length(a)>0 reportFilename=reportFilename(a(end)+1:end);end

file=['report-' regexprep(s.languagefile,'.mat','') '-' reportFilename];
saveFile(file,d)
save(file,'r');
